...
docker run --network=$network_name --name $db_hostname -e MYSQL_ROOT_PASSWORD=$db_password -p 3306:3306 -d mysql:latest
...
do
  echo "this loop will try again until mysql container be ready..."
...
docker run --network=$network_name -d --name api -e SERVER_PORT=$api_port -e DB_HOST=$db_hostname -e DB_USER=$db_user -e DB_NAME=$db_name -e DB_SECRET=$db_password wilsonnetodev/apimodules:dev 
